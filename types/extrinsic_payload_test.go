...
var (
	extrinsicPayloadV3FuzzOpts = extrinsicEraFuzzOpts
)

func TestExtrinsicPayloadV3_EncodeDecode(t *testing.T) {
	f := fuzz.New().NilChance(0)

	for _, opt := range extrinsicPayloadV3FuzzOpts {
		opt(f)
	}

	var ext ExtrinsicPayloadV3
	f.Fuzz(&ext)
	...
	err := ext.Encode(*scale.NewEncoder(buf))
	assert.NoError(t, err)
	assert.True(t, len(buf.Bytes()) > 0)

	err = ext.Decode(*scale.NewDecoder(bytes.NewReader(nil)))
	assert.NotNil(t, err)
}

var examplaryExtrinsicPayload = ExtrinsicPayloadV4{ExtrinsicPayloadV3: ExtrinsicPayloadV3{Method: BytesBare{0x6, 0x0, 0xff, 0xd7, 0x56, 0x8e, 0x5f, 0xa, 0x7e, 0xda, 0x67, 0xa8, 0x26, 0x91, 0xff, 0x37, 0x9a, 0xc4, 0xbb, 0xa4, 0xf9, 0xc9, 0xb8, 0x59, 0xfe, 0x77, 0x9b, 0x5d, 0x46, 0x36, 0x3b, 0x61, 0xad, 0x2d, 0xb9, 0xe5, 0x6c}, Era: ExtrinsicEra{IsImmortalEra: false, IsMortalEra: true, AsMortalEra: MortalEra{First: 0x100, Second: 0x30}}, Nonce: NewUCompactFromUInt(0x1234), Tip: NewUCompactFromUInt(0x5678), SpecVersion: 0x7b, GenesisHash: Hash{0xdc, 0xd1, 0x34, 0x67, 0x1, 0xca, 0x83, 0x96, 0x49, 0x6e, 0x52, 0xaa, 0x27, 0x85, 0xb1, 0x74, 0x8d, 0xeb, 0x6d, 0xb0, 0x95, 0x51, 0xb7, 0x21, 0x59, 0xdc, 0xb3, 0xe0, 0x89, 0x91, 0x2, 0x5b}, BlockHash: Hash{0xde, 0x8f, 0x69, 0xee, 0xb5, 0xe0, 0x65, 0xe1, 0x8c, 0x69, 0x50, 0xff, 0x70, 0x8d, 0x7e, 0x55, 0x1f, 0x68, 0xdc, 0x9b, 0xf5, 0x9a, 0x7, 0xc5, 0x23, 0x67, 0xc0, 0x28, 0xf, 0x80, 0x5e, 0xc7}}, AppID: NewUCompactFromUInt(1), TransactionVersion: 1} //nolint:lll
...
func TestExtrinsicPayload(t *testing.T) {
	...
	era.AsMortalEra = types.MortalEra{First: types.U64(256), Second: types.U64(second)}
